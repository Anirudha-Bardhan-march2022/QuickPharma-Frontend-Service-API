...
const LoginPharma = () => {
  ...
  const userAuthFromDB = () => {
    ...
    if (errorCount == 0) {
      ...
      axios.post(url + "/loginpharma", data).then((response) => {
        const result = response.data;
        if (result.status === "success") {
          console.log(result);
          alert("Login Successfull");
          Cookies.set("firstName", result.data.firstName);
          sessionStorage.setItem("pharmacistId", result.data.pharmacistId);
          localStorage.setItem("email", email);
          history.push("/homepharma");
  ...
  return (
    <div className="card bg-light bgimpp">
      <div className="wrapper">
        <div className="formp-signin loginp-container">
          <h2 className="formp-signin-heading">Sign In</h2>
